...
#' subtitle: 300 - figure for regression coefficients
#' author: Julien Riou
#' date: 2023-11-01
#' ---

fn_300_fig_regression = function(coefs, fade_overlap=TRUE, ...) {
  # labels
  ...
  if(FALSE) {
    source("~/en_cours/2024_pfas1/shes_pilot_pfas/R/fn_000_additional_controls.R")
    ...
    dd_ = coefs %>% 
      ...
      dplyr::mutate(away=ifelse(exp_beta_upb<1 | exp_beta_lob>1,"95% CrI away from 1","95% CrI includes 1")) 
    dd_ %>%  
      dplyr::select(parameter,parameter2_lab) %>% 
      filter(is.na(parameter2_lab),!grepl("hu_",parameter)) %>% 
      mutate(parameter2_lab=as.character(parameter2_lab)) %>% 
      replace_na(replace=list(parameter2_lab="' '")) %>% 
      distinct() %>% 
      write_delim(file="misc/add_controls.R",delim = "=",eol = ",\n")
  }
  ...
  # options
  g = g +
    facet_wrap(~dependent_variable_lab,nrow=2) +
