...
fn_207_multinomial = function(dat_in, covariates, multivariable, regularization=FALSE, adj=NULL, multiple_runs=TRUE, impute=NULL, monotonic=NULL, ...) {
  
  # format data ----
  dat_ = dat_in %>% 
    dplyr::rename(y=pfas_cluster) %>% 
    dplyr::mutate(y=factor(y,ordered=FALSE)) %>% 
    dplyr::select(y,all_of(covariates),all_of(impute),all_of(adj))
  
  # generate priors ----
  # weakly informative prior normal(0,1) on the log scale
  prior_ = prior("normal(0,1)", class="b", dpar="mu2Intermediate") +
    prior("normal(0,1)", class="b", dpar="mu3High")
  # horseshoe prior for regularization (see Piironen and Vehtari 2017, we expect 20% of parameters away from 0)
  if(regularization) {
    prior_ = prior(horseshoe(df = 1, par_ratio = 0.1), class="b", dpar="mu2Intermediate") +
      prior(horseshoe(df = 1, scale_global = 0.1), class="b", dpar="mu3High")
  }
  ...
  pars_ = res_ %>% 
    ...
    dplyr::mutate(type="Odds ratio",
                  ...
                  prior=prior_[1,1]) %>% 
    tidyr::separate(parameter,into=c("dependent_variable","parameter"),sep="Intermediate_|High_")
