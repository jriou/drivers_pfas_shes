...
#' subtitle: 200 - Hurdle gamma regression
#' author: Julien Riou
#' date: 2023-10-19
#' ---

fn_200_hurdle_gamma = function(dat_in, dependent, covariates, multivariable, regularization=FALSE, adj=NULL, multiple_runs=TRUE, impute=NULL, monotonic=NULL, ...) {
  
  # format data ----
  y_name = paste0(dependent,"_quant")
  dat_ = dat_in %>% 
    dplyr::rename(y=all_of(y_name)) %>% 
    dplyr::mutate(y=if_else(is.na(y),0,y)) %>% 
...
# function to fix a mistake in exp_beta in earlier versions
fn_200_hurdle_gamma_correction = function(res_) {
  tt_ = res_ %>% 
    dplyr::mutate(
      Estimate=if_else(grepl("hu_",parameter),-log(Estimate),log(Estimate)),
      `l-95% CI`=if_else(grepl("hu_",parameter),-log(`l-95% CI`),log(`l-95% CI`)),
      `u-95% CI`=if_else(grepl("hu_",parameter),-log(`u-95% CI`),log(`u-95% CI`))) %>% 
    dplyr::mutate(
      exp_beta=if_else(grepl("hu_",parameter),exp(-Estimate),exp(Estimate)),
      exp_beta_lob=if_else(grepl("hu_",parameter),exp(-`u-95% CI`),exp(`l-95% CI`)),
      exp_beta_upb=if_else(grepl("hu_",parameter),exp(-`l-95% CI`),exp(`u-95% CI`))
...
fn_200_hurdle_gamma_correction2 = function(res_) {
  res_$type[grepl("Relative change in measure",res_$type)] <- "Fold-change"
  return(res_)
}
