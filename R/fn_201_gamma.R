...
#' subtitle: 201 - Gamma regression
#' author: Julien Riou
#' date: 2023-10-19
#' ---

fn_201_gamma = function(dat_in, dependent, covariates, multivariable, regularization=FALSE, adj=NULL, multiple_runs=TRUE, impute=NULL, monotonic=NULL, ...) {
  
  # format data ----
  y_name = paste0(dependent,"_quant")
  dat_ = dat_in %>% 
    dplyr::rename(y=y_name) %>% 
    dplyr::select(y,all_of(covariates),all_of(impute),all_of(adj))
  
  # generate priors ----
  prior_ = set_prior("normal(0,1)", class="b") # weakly informative prior normal(0,5) on the log scale
  if(multivariable & regularization) {
    prior_ = set_prior(horseshoe(df = 1, par_ratio = 0.2)) # see Piironen and Vehtari 2017 (we expect 20% of parameters away from 0)
  }
